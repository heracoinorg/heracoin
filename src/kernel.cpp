...
// Get time weight
int64_t GetWeight(int64_t heraervalBeginning, int64_t heraervalEnd)
{
    return min(heraervalEnd - heraervalBeginning - nStakeMinAge, (int64_t)nStakeMaxAge);
}
...
static int64_t GetStakeModifierSelectioheraervalSection(int nSection)
{
...
static int64_t GetStakeModifierSelectioheraerval()
{
    int64_t nSelectioheraerval = 0;
    for (int nSection = 0; nSection < 64; nSection++) {
        nSelectioheraerval += GetStakeModifierSelectioheraervalSection(nSection);
    }
    return nSelectioheraerval;
}

// select a block from the candidate blocks in vSortedByTimestamp, excluding
// already selected blocks in vSelectedBlocks, and with timestamp up to
// nSelectioheraervalStop.
static bool SelectBlockFromCandidates(
    vector<pair<int64_t, uint256> >& vSortedByTimestamp,
    map<uint256, const CBlockIndex*>& mapSelectedBlocks,
    int64_t nSelectioheraervalStop,
    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*)0;
    BOOST_FOREACH (const PAIRTYPE(int64_t, uint256) & item, vSortedByTimestamp) {
        ...
        if (fSelected && pindex->GetBlockTime() > nSelectioheraervalStop)
            break;
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    int64_t nSelectioheraerval = GetStakeModifierSelectioheraerval();
    int64_t nSelectioheraervalStart = (pindexPrev->GetBlockTime() / getIntervalVersion(fTestNet)) * getIntervalVersion(fTestNet) - nSelectioheraerval;
    const CBlockIndex* pindex = pindexPrev;

    while (pindex && pindex->GetBlockTime() >= nSelectioheraervalStart) {
    ...
    int64_t nSelectioheraervalStop = nSelectioheraervalStart;
    map<uint256, const CBlockIndex*> mapSelectedBlocks;
    for (int nRound = 0; nRound < min(64, (int)vSortedByTimestamp.size()); nRound++) {
        // add an interval section to the current selection round
        nSelectioheraervalStop += GetStakeModifierSelectioheraervalSection(nRound);

        // select a block from the candidates of current round
        if (!SelectBlockFromCandidates(vSortedByTimestamp, mapSelectedBlocks, nSelectioheraervalStop, nStakeModifier, &pindex))
        ...
        if (fDebug || GetBoolArg("-printstakemodifier", false))
            LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n",
                nRound, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nSelectioheraervalStop).c_str(), pindex->nHeight, pindex->GetStakeEntropyBit());
    }
...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    int64_t nStakeModifierSelectioheraerval = GetStakeModifierSelectioheraerval();
    const CBlockIndex* pindex = pindexFrom;
    CBlockIndex* pindexNext = chainActive[pindexFrom->nHeight + 1];

    // loop to find the stake modifier later by a selection interval
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectioheraerval) {
        if (!pindexNext) {
            // Should never happen
