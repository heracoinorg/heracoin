...
{
    /* current difficulty formula, hera - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
...
unsigned int GetLegacyNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock, bool fProofOfStake)
{
    ...
    // hera coin: target change every block
    // hera coin: retarget with exponential moving toward target spacing
    uint256 bnNew;
    bnNew.SetCompact(pindexPrev->nBits);

    if (pindexLast->nHeight+1 >= nHardForkBlock)
    {
        int64_t heraerval = nTargetTimespan / nTargetSpacing;
        bnNew *= ((heraerval - 1) * nTargetSpacing + nActualSpacing + nActualSpacing);
        bnNew /= ((heraerval + 1) * nTargetSpacing);
    }
    else
    {
        int64_t heraerval = nTargetTimespan / nTargetSpacingOld;
        bnNew *= ((heraerval - 1) * nTargetSpacingOld + nActualSpacing + nActualSpacing);
        bnNew /= ((heraerval + 1) * nTargetSpacingOld);
    }
